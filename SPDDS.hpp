...
class SPDDS_solver{
    ...
    */
    ...
    public:
    SPDDS_solver(int argc, char *argv[]){
    ...
    void Init(double global_parameters[4], unsigned int N_centers, double superposition_coefficent, 
            ...
            for(unsigned int h_index = 0; h_index <= N_centers-1; h_index++){
                for(unsigned int v_index = 0; v_index <= N_centers-1; v_index++){
                    ...
                    knot_centinel = aux_center.Init(R,bvp,fac,aux_vec,h_index,v_index,N_knots_per_circle,knot_centinel,N_centers-1,N_centers-1);
    ...
    void Test_Interpolator_Serial(BVP bvp){
        FILE *pFile;
        Center aux_center;
        ...
        if(myid == server){
            ...
            for(unsigned int knot_index = 0; knot_index < knots.size(); knot_index ++){
                aux_center = (*knots[knot_index][0]);
                //angle = aux_center.theta.begin()->second + ((--aux_center.theta.end())->second- aux_center.theta.begin()->second)*(float)std::rand()/RAND_MAX;
                angle = aux_center.theta[knot_index] + 2*M_PI/192.0;
                point.resize(2);
                point[0] = aux_center.center_vec[0] + aux_center.Get_radius()*cos(angle);
                point[1] = aux_center.center_vec[1] + aux_center.Get_radius()*sin(angle);
                aux_center.Test_Center_Interpolator(point,bvp);
                value = 0.0;
                for(std::map<unsigned int, double>::iterator it_G = aux_center.GN.begin();
                    it_G != aux_center.GN.end(); it_G ++){
                    aux_vec.resize(2);
                    aux_center.Get_knot_position(it_G->first,aux_vec);
                    value += it_G->second * bvp.u.Value(aux_vec,0.0);
                }
                fprintf(pFile, "%u,[%u %u],%f,%f,%f,%f,%e\n",knot_index,aux_center.index[0],aux_center.index[1],
                        point[0], point[1], bvp.u.Value(point, 0.0), value, bvp.u.Value(point, 0.0) - value);
            }
            fclose(pFile);
        }
    };
    void Test_Interpolator_Parallel(BVP bvp){
        FILE *pFile;
        Eigen::VectorXd point, aux_vec;
        double angle, value;
        if(myid == server){
    ...
    void Solve_Interfaces_MC(BVP bvp, double discretization, unsigned int N_tray){
        double start = MPI_Wtime();
        FILE *pFile;
        if(myid == server){
